...
import { ChevronLeft, ChevronRight, Printer, List, Edit3, Sparkles, MessageSquare, Mic } from 'lucide-react';
...
export const BookReader: React.FC<BookReaderProps> = ({
  ...
  const activeChapter = hasChapters ? chapters[selectedChapterIndex] : null;
    ...
    <div className="flex-1 flex flex-col space-y-4 max-w-5xl mx-auto w-full pb-20 select-none">
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-3.5 flex flex-wrap items-center justify-between gap-3 shadow-md">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <span className="font-book font-black text-sm sm:text-base text-rose-200 block sm:inline">
              {userName} Könyve • {activeChapter ? activeChapter.volumeName : 'I. Kötet (Tiszta lap)'}
            </span>
            {hasChapters && (
        ...
        <div className="flex items-center space-x-2.5">
          <button
            ...
            className="bg-amber-800/90 hover:bg-amber-700 disabled:opacity-40 disabled:cursor-not-allowed text-amber-100 px-3.5 py-1.5 rounded-lg text-xs font-bold flex items-center space-x-1.5 transition shadow"
          >
          ...
          {hasChapters && (
            <div className="flex items-center bg-slate-800 rounded-lg p-0.5 text-xs text-slate-200 border border-slate-700">
              ...
              <span className="px-2 text-[11px] text-slate-400 font-mono font-semibold">
                {activeChapter?.pageNumber}-{Number(activeChapter?.pageNumber) + 1}. oldal
              </span>
              <button
                onClick={handleNext}
        ...
        <div className="w-full max-w-4xl bg-stone-900 rounded-xl p-2 sm:p-4 shadow-2xl border border-stone-800 flex justify-center">
          ...
          <div className="w-full grid grid-cols-1 md:grid-cols-2 rounded-lg shadow-2xl overflow-hidden relative border border-amber-950/40">
            ...
            {hasChapters && activeChapter ? (
              <>
                ...
                <div className="book-paper p-6 sm:p-9 text-stone-900 flex flex-col justify-between min-h-[490px] relative border-r border-stone-300/60">
                  <div className="flex justify-between items-center text-[10px] text-stone-500 font-book uppercase tracking-widest pb-3 border-b border-stone-200">
                    <span>{userName} Élettörténete</span>
                ...
                <div className="book-paper p-6 sm:p-9 text-stone-900 flex flex-col justify-between min-h-[490px] relative">
                  ...
                  <div className="flex justify-between items-center text-xs text-stone-500 font-book pt-3 border-t border-stone-200">
                    <button
                      onClick={() => alert("✍️ Fejezetszerkesztő: Újragenerálhatsz részeket, módosíthatod a szöveget vagy fotót cserélhetsz.")}
              ...
              /* EMPTY BOOK STATE (When user is brand new) */
              <>
                {/* Empty Left Page */}
                <div className="book-paper p-6 sm:p-10 text-stone-900 flex flex-col justify-between min-h-[490px] relative border-r border-stone-300/60">
                  <div className="flex justify-between items-center text-[10px] text-stone-500 font-book uppercase tracking-widest pb-3 border-b border-stone-200">
                    <span>{userName} Életkönyve</span>
                    <span>1. Oldal</span>
                  </div>

                  <div className="my-auto py-6 text-center">
                    <div className="w-12 h-12 rounded-full bg-rose-900/10 text-rose-900 flex items-center justify-center mx-auto mb-3 font-serif text-2xl font-bold">
                      I.
                    </div>
                    <h2 className="font-book text-2xl sm:text-3xl font-bold text-stone-900 leading-tight mb-3">
                      A Tiszta Fehér Lap
                    </h2>
                    <p className="font-book text-sm sm:text-base leading-relaxed text-stone-700 text-justify italic mt-4">
                      „Minden ember élete egy megíratlan könyv. A te könyved ma kezdődik el. Nem kell írónak lenned: a te egyetlen feladatod a mesélés.”
                    </p>
                    <p className="font-book text-xs text-stone-600 text-justify mt-3 leading-relaxed">
                      Kattints a FateAI csevegőre, nyisd meg a mikrofont, és mesélj egy kedves gyermekkori emlékedről, a szüleidről vagy a fiatalságodról. Az AI megírja belőle a legelső fejezetedet!
                    </p>
                  </div>

                  <div className="flex justify-between items-center text-xs text-stone-500 font-book pt-3 border-t border-stone-200">
                    <span>1</span>
                    <span className="text-[10px] italic">Kezdet</span>
                  </div>
                </div>

                {/* Empty Right Page */}
                <div className="book-paper p-6 sm:p-10 text-stone-900 flex flex-col justify-between min-h-[490px] relative">
                  <div className="hidden md:block absolute left-0 top-0 bottom-0 w-8 book-spine-shadow pointer-events-none"></div>

                  <div className="flex justify-between items-center text-[10px] text-stone-500 font-book uppercase tracking-widest pb-3 border-b border-stone-200">
                    <span>Első Fejezet Váró</span>
                    <span>FateAI</span>
                  </div>

                  <div className="my-auto py-6 flex flex-col items-center text-center">
                    {/* Empty Polaroid placeholder */}
                    <div className="bg-white p-4 pb-6 shadow-md border border-dashed border-stone-300 rotate-1 max-w-[220px] my-2">
                      <div className="w-36 h-36 bg-stone-100 flex flex-col items-center justify-center text-stone-400 text-xs rounded-sm">
                        <span className="text-2xl mb-1">📷</span>
                        <span className="text-[10px] font-sans font-semibold">Itt lesz az első fotód</span>
                      </div>
                      <p className="font-hand text-xs text-stone-500 text-center mt-3">
                        „Az első fejezet illusztrációja”
                      </p>
                    </div>
                    ...
                    {onNavigateToAi && (
                      <button
                        ...
                        className="mt-4 px-4 py-2 rounded-lg bg-rose-900 hover:bg-rose-800 text-white font-bold text-xs flex items-center space-x-2 shadow transition active:scale-95"
                      >
                        <Mic className="w-3.5 h-3.5 text-rose-300" />
                        <span>Kezdj el mesélni a FateAI-nak →</span>
                      </button>
                    )}
                  </div>
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-3.5 text-xs flex flex-wrap items-center justify-between gap-2 text-slate-400">
        <div className="flex items-center space-x-2">
          ...
          <span>
            {hasChapters
              ? 'A fejezetek drag-and-drop rendezhetők és tematikus kötetekbe csoportosíthatók.'
              : 'A beszélgetések befejezésekor az AI automatikusan felajánlja a fejezet elkészítését.'}
          </span>
        </div>
        <span className="text-slate-300 font-semibold">
          {hasChapters
            ? `Teljes könyv: ${chapters.length} fejezet • ~${chapters.length * 12 + 10} oldal`
            : '0 megírt fejezet • Indulásra kész'}
        </span>
      </div>
